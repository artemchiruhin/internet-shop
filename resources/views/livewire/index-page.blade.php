<div>
    ...
    <div @class(['modal','fade','show' => $productDeletedFromComparsion]) id="exampleModalCenter" tabindex="-1" aria-modal="true" role="dialog" @if($productDeletedFromComparsion) style="display: block" @endif>
        <div class="modal-dialog modal-dialog-centered" style="max-width: 800px">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @if(count($productsForComparsion) > 0)
                    <div class="d-flex justify-content-between">
                        @foreach($productsForComparsion as $i => $product)
                        <div class="card mb-3 p-0 mx-1" style="flex: 1">
                            <img src="{{ get_image_path($product['image'])  }}" class="card-img-top" alt="{{ $product['name'] }}">
                            <div class="card-body d-flex flex-column">
                                <h5 class="card-title">{{ $product['name'] }}</h5>
                                <p class="card-text">{{ $product['description'] }}</p>
                                <p class="card-text">Категория: <span class="text-primary">{{ $product['category']['name'] }}</span></p>
                                <p class="card-text">Цена: <span class="text-primary">Цена {{ number_format($product['price'], 2, ',', ' ') }} р.</span></p>
                            </div>
                            <div class="card-buttons mt-auto">
                                <button type="button" class="btn btn-danger d-block mt-2 w-100" wire:click="deleteFromComparison({{ $i }})" wire:key="{{ $i }}">Удалить</button>
                            </div>
                        </div>
                        @endforeach
                    </div>
